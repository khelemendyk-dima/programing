...
public class DBManager {
	...
	public List<User> getTeamUsers(Team team) throws DBException {
		...
		try (Connection connection = DriverManager.getConnection(CONNECTION_URL);
			 PreparedStatement pStatement = connection.prepareStatement(GET_TEAM_USERS);) {
			pStatement.setString(1, team.getName());
			ResultSet resultSet = pStatement.executeQuery();
			while (resultSet.next()) {
				String name = resultSet.getString(2);
				teamUsers.add(User.createUser(name));
			}
		} catch (SQLException e) {
			e.printStackTrace();
			throw new DBException();
	...
	public boolean deleteTeam(Team team) throws DBException {
		boolean isDeleted;
		try (Connection connection = DriverManager.getConnection(CONNECTION_URL);
			 PreparedStatement pStatement = connection.prepareStatement(DELETE_TEAM)) {
			pStatement.setString(1, team.getName());
			isDeleted = pStatement.execute();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public boolean updateTeam(Team team) throws DBException {
		boolean isUpdated;
		try (Connection connection = DriverManager.getConnection(CONNECTION_URL);
			 PreparedStatement pStatement = connection.prepareStatement(UPDATE_TEAM)) {
			pStatement.setString(1, team.getName());
			pStatement.setString(2, "A");
			isUpdated = pStatement.execute();
		} catch (SQLException e) {
			e.printStackTrace();
			throw new DBException();
		}
