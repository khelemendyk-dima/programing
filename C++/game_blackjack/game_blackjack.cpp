...
class Card
{
	public:
		...
		// перегружаем оператоp <<, чтобы можно было отправить объект 
		// типа Card в стандартный поток вывода
		friend ostream &operator<<(ostream &os, const Card &aCard);
		Card(rank r = ACE, suit s = SPADES, bool ifu = true);
		// возвращает значение каpты от 1 до 11
		int GetValue() const;
...
{
	// если карта перевернута лицом вниз
	int value = 0;
	if (m_IsFaceUp)
	{
...
int Hand::GetTotal() const
{
	...
	if (m_Cards.empty())
		return 0;
	// если первая карта имее значение 0, то она лежит рубашкой вверх;
	// вернуть значение 0
...
class House: public GenericPlayer
{
	public:
		...
		virtual ~House();
		// показывает, хочет ли игрок продолжить брать карты
		virtual bool IsHitting() const;
...
{
	cout << endl;
	// продолжает раздавать карты до тез пор, пока у игрока не случается
...
void Game::Play()
{
	// рaздает каждому по две стартовые карты
	...
	{
		// сравнивает суммы очков всех оставшився игроков с суммой очков дилера
		for (pPlayer = m_Players.begin(); pPlayer != m_Players.end(); pPlayer++)
		{
			if (!(pPlayer->IsBusted()))
			{
